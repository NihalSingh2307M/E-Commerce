      ...
      <div className='flex flex-col sm:grid grid-cols-[3fr_1fr_1fr] gap-12 sm:gap-10 py-14 sm:py-16'>
        <div>
          ...
          <div className='flex gap-4 mt-6'>
            {['Instagram', 'Pinterest', 'Twitter'].map(s => (
              <button key={s} className='jost text-[10px] tracking-[0.2em] uppercase text-[#B8956A] hover:text-[#1C1C1C] transition-colors duration-200'>{s}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <ul className='flex flex-col gap-2.5'>
            {[['Home', '/'], ['About Us', '/about'], ['Collection', '/collection'], ['Privacy Policy', '/']].map(([label, path]) => (
              <li key={label}>
                <Link to={path} className='jost text-[#6B6560] text-sm font-light hover:text-[#B8956A] transition-colors duration-200'>{label}</Link>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className='border-t border-[#E2D9CC] py-5 flex flex-col sm:flex-row items-center justify-between gap-2'>
        ...
        <div className='flex gap-6'>
          {['Terms', 'Privacy', 'Cookies'].map(t => (
            <button key={t} className='jost text-[10px] tracking-[0.2em] uppercase text-[#6B6560] hover:text-[#B8956A] transition-colors'>{t}</button>
          ))}
        </div>
      </div>
    </footer>
