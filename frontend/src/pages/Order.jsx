      ...
      {orderData.length === 0 ? (
        ...
        <div className="flex flex-col gap-0">
          {orderData.map((item, index) => (
            <div
              key={index}
              className="py-5 sm:py-6 border-t border-[#E2D9CC] flex flex-col md:flex-row md:items-center md:justify-between gap-4 sm:gap-6"
            >
              ...
              <div className="flex items-center justify-between md:justify-end gap-6 md:min-w-[200px]">
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full shrink-0 ${STATUS_COLOR[item.status] ?? 'bg-[#B8956A]'}`} />
